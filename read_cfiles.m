function [txt,ctab, colors] = read_cfiles(CFile);
%
% Syntax :
% [txt,ctab, colors] = read_cfiles(CFile);
%
    ...
    try
        ...
        try
            ...
            if isempty(txt)
                ...
                if sum(ctab.table(:))~=0
                    [~,ordId] = ismember(txt,ctab.table(:,5));
                    colors = ones(length(txt),3);
                    index = find(ordId);
                    colors(index,:) = ctab.table(ordId(index),1:3)/255;
                end
