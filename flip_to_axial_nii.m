function outputs = flip_to_axial_nii(filenames,owbool,intorder)

%FLIP_TO_AXIAL flips sagittal or coronal images to axial view.
%
% USAGE: flip_to_axial_nii(filenames,owbool,intorder)
% 
...
if ~exist('intorder','var')
    intorder = 7;
end 
for j= 1:size(filenames,1)
    tempName = deblank(filenames(j,:));
    [pathstr,name] = fileparts(tempName);
    V = spm_vol(tempName);
    if length(V)>1
        for i = 1:length(V),
            Vn(i) = V(i);
            Vn(i).fname = fullfile(pathstr,['f' name '.nii']);
            R = getrot(V(i).mat(1:3,1:3));
            if abs(R) == eye(3), outputs{j} = tempName; continue, end %#ok
            ...
            Vn(i).dim(1:3) = abs(R(1:3,1:3)*V(i).dim(1:3)')';
            Vn(i).mat = V(i).mat*inv(R); if det(Vn(i).mat)>0,
            Vn(i).mat = Vn(i).mat*inv([diag([-1 1 1]) (Vn(i).dim(1)+1)*eye(3,1); zeros(1,3) 1]); end
        end
        Vn = spm_create_vol(Vn);

        for i = 1:length(Vn),
           for z = 1:Vn(i).dim(3),
                A = spm_slice_vol(V(i),V(i).mat\Vn(i).mat*spm_matrix([0 0 z]),Vn(i).dim(1:2),intorder);
                spm_write_plane(Vn(i),A,z);
            end
        end
        fclose all;
        outputs{j} = fullfile(pathstr,['f' name '.nii']); %#ok
        if nargin && owbool,
            delete(fullfile(pathstr,[name '.nii']));
            movefile(fullfile(pathstr,['f' name '.nii']),fullfile(pathstr,[name '.nii']));
            outputs{j} = fullfile(pathstr,[name '.nii']); %#ok
        end
        ...
        for i = 1:length(V),
            ...
            R = getrot(V(i).mat(1:3,1:3));
            if abs(R) == eye(3), outputs{j} = deblank(filenames(i,:)); continue, end %#ok
            ind = find(R); if length(ind) ~= 3 && abs(det(R)) == 1,
                error('Imposible to get axial plane normal direction:'); end
            [r,c] = find(R<0); R = [R zeros(3,1);zeros(1,3) 1];
            for j = 1:length(c), R(r(j),4) = V(i).dim(c(j))+1; end
            ...
            Vn = spm_create_vol(Vn);
            for z = 1:Vn.dim(3),
                A = spm_slice_vol(V(i),V(i).mat\Vn.mat*spm_matrix([0 0 z]),Vn.dim(1:2),7);
            ...
            try %#ok
                usrdata = dti_get_dtidata(outputs{j});
                usrdata.mat = Vn.mat;
                dti_get_dtidata(outputs{j},usrdata);
            end
        end
    end
end
